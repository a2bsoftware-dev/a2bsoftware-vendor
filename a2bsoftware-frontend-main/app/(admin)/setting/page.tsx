...
import { 
  Settings, Image, Link, Check, AlertTriangle, 
...
export default function SettingsPage() {
    ...
    try {
      ...
      if (res.ok) {
        ...
        if (data.success && data.settings) {
          ...
          setForm({
            large_image: loaded.large_image || "",
            small_image: loaded.small_image || "",
            fevico_image: loaded.fevico_image || "",
            login_image: loaded.login_image || "",
            survey_start_image: loaded.survey_start_image || "",
            survey_end_image: loaded.survey_end_image || "",
            loader_image: loaded.loader_image || "",
            showCopy: loaded.showCopy || false,
            cpi: loaded.cpi || "",
            show_client_api: loaded.show_client_api || false,
            show_setting: loaded.show_setting || false
          });
        }
      }
    } catch (err) {
    ...
    try {
      ...
      if (res.ok) {
        ...
        if (data.success) {
          ...
          setForm((prev: any) => ({ ...prev, [key]: mediaUrl }));
  ...
  const renderUploadRow = (label: string, fieldKey: string, description: string) => {
