...
import { 
  FolderKanban, ArrowLeft, Loader2, Save,
...
export default function AddEditProjectPage() {
  ...
  const loadProjectInitData = async () => {
    setLoading(true);
    try {
      ...
      if (res.ok) {
        ...
        if (data.success) {
          ...
          if (data.projectData) {
            ...
            const parent = data.projects.find((proj: any) => String(proj.id) === String(p.parentProjectId));
            if (parent) {
  ...
  const filteredProjects = () => {
    return options.projects.filter((p: any) =>
      p.projectName.toLowerCase().includes(searchProjectText.toLowerCase())
    );
  };

  const selectParentProject = (project: any) => {
    setFormData(prev => ({
      ...prev,
          ...
          <Card className="col-span-2 md:col-span-2 border-zinc-200 shadow-sm bg-white dark:bg-zinc-900">
            <CardContent className="p-4">
              ...
              <div className="space-y-1.5">
                {options.securityChecklist.map((c: any) => (
            ...
            <CardContent className="pt-6 pb-6 space-y-4">
              ...
              <div className="space-y-1.5">
                ...
                >
                  ...
                  {options.projectManagers.map((pm: any) => (
              ...
              <div className="space-y-1.5">
                ...
                >
                  ...
                  {options.salesManagers.map((sm: any) => (
