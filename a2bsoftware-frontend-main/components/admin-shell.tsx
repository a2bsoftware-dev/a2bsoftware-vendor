...
export default function AdminShell({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    // 1. Sync the logged-in Keycloak user into the local users table, then
    // fetch authenticated user details from the backend (Spring Boot owns
    // the session/token check now — this call carries the httpOnly cookies).
    apiFetch("/api/auth/sync", { method: "POST" }).catch((err) => {
      console.error("Error syncing user from Keycloak", err);
    });
