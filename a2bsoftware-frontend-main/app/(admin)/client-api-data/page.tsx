    ...
    <div className="p-3 bg-zinc-50 dark:bg-zinc-950 rounded-lg border border-zinc-150 space-y-2">
      ...
      <div className="space-y-1.5">
        ...
        <div className="grid grid-cols-1 gap-1">
          {qual.options.map((opt: any, oidx: number) => (
            <div
              key={oidx}
...
export default function ClientApiDataPage() {
  ...
  const [qualsViewProject, setQualsViewProject] = useState<any>(null);
  const [qualsViewList, setQualsViewList] = useState<any[]>([]);
  ...
  const openEditCampaignModal = async (proj: any) => {
    setCampaignData({
      id: proj.id,
  ...
  const openQualificationsView = async (project: any) => {
